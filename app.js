...
function loadDatabase() {
    ...
    if (!localStorage.getItem('fms_flushed_v3')) {
        localStorage.clear();
        localStorage.setItem('fms_flushed_v3', 'true');
    }
...
function logAdminAction(actionType, entityType, entityId, details) {
    ...
    const action = {
        ...
        action_type: actionType, // STUDENT_CREATED, STUDENT_EDITED, STUDENT_DELETED, FINE_CREATED, FINE_EDITED, FINE_STATUS_CHANGED, FINE_DELETED, BULK_IMPORT, FILE_EXPORT
        entity_type: entityType, // STUDENT, FINE, BULK_UPLOAD
        entity_id: entityId,
        timestamp: new Date().toISOString(),
        details: typeof details === 'object' ? JSON.stringify(details) : details
    };
    db.actionLog.unshift(action); // newest first
    saveTable(DB_KEYS.ADMIN_ACTION_LOG, db.actionLog);
    
    // If we are currently viewing the audit logs page, refresh it
...
function logFineEdit(fineId, changedField, oldValue, newValue, changeType, actionNote = '', save = true) {
    const audit = {
        id: generateUUID(),
        fine_id: fineId,
        changed_field: changedField, // 'violation_category', 'monetary_penalty', 'red_flags', 'status', 'notes'
        ...
        change_type: changeType, // CREATED, EDITED, STATUS_CHANGED, DELETED
...
function seedStudentsAndFines() {
    ...
    const summerCats = db.violations.filter(v => v.cohort === 'Summer');
    const finalCats = db.violations.filter(v => v.cohort === 'Final');
...
function initializeDashboard() {
    // 1. Calculate Metrics for Summer Cohort
    const summerStudents = db.students.filter(s => s.cohort === 'Summer' && !s.is_deleted);
    const summerFines = db.fines.filter(f => f.cohort === 'Summer' && !f.is_deleted);
    
    const sipFinedStudentIds = new Set(summerFines.map(f => f.student_id));
    document.getElementById('sipStudentsFined').innerText = sipFinedStudentIds.size;
    
    const sipOpen = summerFines.filter(f => f.status === 'OPEN');
    document.getElementById('sipOpenFines').innerText = sipOpen.length;
    
    const sipClosed = summerFines.filter(f => f.status === 'CLOSED');
    document.getElementById('sipClosedFines').innerText = sipClosed.length;
    
    const sipCarried = summerFines.filter(f => f.status === 'CARRIED_FORWARD');
    document.getElementById('sipCarriedFines').innerText = sipCarried.length;
    
    const sipPendingSum = sipOpen.reduce((sum, f) => sum + (f.monetary_penalty_amount || 0), 0);
    document.getElementById('sipPendingAmount').innerText = formatINR(sipPendingSum);

    // 2. Calculate Metrics for Final Cohort
    const finalStudents = db.students.filter(s => s.cohort === 'Final' && !s.is_deleted);
    const finalFines = db.fines.filter(f => f.cohort === 'Final' && !f.is_deleted);
    
    const finalFinedStudentIds = new Set(finalFines.map(f => f.student_id));
    document.getElementById('finalStudentsFined').innerText = finalFinedStudentIds.size;
    
    const finalOpen = finalFines.filter(f => f.status === 'OPEN');
    document.getElementById('finalOpenFines').innerText = finalOpen.length;
    
    const finalClosed = finalFines.filter(f => f.status === 'CLOSED');
    document.getElementById('finalClosedFines').innerText = finalClosed.length;
    
    const finalCarried = finalFines.filter(f => f.status === 'CARRIED_FORWARD');
    document.getElementById('finalCarriedFines').innerText = finalCarried.length;
    
    const finalPendingSum = finalOpen.reduce((sum, f) => sum + (f.monetary_penalty_amount || 0), 0);
    document.getElementById('finalPendingAmount').innerText = formatINR(finalPendingSum);

    // 3. Render Top Categories Chart
    renderDashboardChart();

    // 4. Render Action History Log
...
function loadStudentList() {
    ...
    // 3. Sort students
    filtered.sort((a, b) => {
        let valA = a[studentSortField].toString().toLowerCase();
        let valB = b[studentSortField].toString().toLowerCase();
    ...
    paginatedData.forEach(student => {
        ...
        if (studentTab === 'active') {
            actionButtons = `
                <button class="btn btn-outline btn-sm" onclick="openEditStudentModal('${student.id}')" title="Edit Profile">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                <button class="btn btn-outline btn-sm" style="color: var(--color-danger); border-color: rgba(239,68,68,0.2);" onclick="softDeleteStudent('${student.id}')" title="Soft Delete">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                </button>
                <button class="btn btn-secondary btn-sm" onclick="drilldownToStudentFines('${student.id}')">View Fines</button>
            `;
        } else {
            actionButtons = `
                <button class="btn btn-outline btn-sm" style="color: var(--color-success); border-color: rgba(16,185,129,0.2);" onclick="restoreStudent('${student.id}')">
                    <span>Restore</span>
                </button>
                <button class="btn btn-danger btn-sm" onclick="permanentPurgeStudent('${student.id}')" title="Permanently Purge">
                    <span>Purge</span>
                </button>
            `;
        }
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge ${student.cohort === 'Summer' ? 'badge-open' : 'badge-carried'}">${student.cohort}</span></td>
            <td>
...
function loadFineList() {
    ...
    // 3. Tab Badge updates
    const totalOpen = db.fines.filter(f => !f.is_deleted && f.status === 'OPEN').length;
    ...
    paginatedData.forEach(fine => {
        ...
        if (fineTab === 'DELETED') {
            actions = `
                <button class="btn btn-outline btn-sm" style="color: var(--color-success); border-color: rgba(16,185,129,0.2);" onclick="restoreFine('${fine.id}')">Restore</button>
            `;
        } else {
            actions = `
                <button class="btn btn-outline btn-sm" onclick="openFineDetailsModal('${fine.id}')" title="View Details/Audit">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"/><circle cx="12" cy="12" r="3"/></svg>
                </button>
                <button class="btn btn-outline btn-sm" onclick="openEditFineModal('${fine.id}')" title="Edit Record">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>
                </button>
                <button class="btn btn-secondary btn-sm" onclick="openIndividualStatusModal('${fine.id}')" title="Update Status">Status</button>
                <button class="btn btn-outline btn-sm" style="color: var(--color-danger); border-color: rgba(239,68,68,0.2);" onclick="softDeleteFine('${fine.id}')" title="Delete">
                    <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V6m3 0V4a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
                </button>
            `;
        }
        ...
        tr.innerHTML = `
            ...
            <td><span class="badge ${fine.cohort === 'Summer' ? 'badge-open' : 'badge-carried'}">${fine.cohort}</span></td>
...
safeAddListener('fineForm', 'submit', function(e) {
    ...
    if (mode === 'ADD') {
        ...
        if (oldCat !== catId) {
            fine.violation_category_id = catId;
            logFineEdit(fine.id, 'violation_category', oldCat, catId, 'EDITED', 'Category modified');
            changed = true;
        }
        if (oldAmount !== amount) {
...
function openFineDetailsModal(id) {
    ...
    document.getElementById('fdCohort').innerText = student.cohort === 'Summer' ? 'Summer Internship Placement' : 'Final Placement';
...
function executeExport() {
    ...
    const currentCohort = document.getElementById('fineCohortFilter').value;
    const currentSearch = document.getElementById('fineSearchInput').value.trim().toLowerCase();
    const currentRedFlag = document.getElementById('fineRedFlagFilter').value;
    const currentMultiCat = document.getElementById('fineMultipleCategoriesCheckbox').checked;
    ...
    const filtered = db.fines.map(f => {
        ...
        if (fineCategoryFilter.length > 0 && !fineCategoryFilter.includes(f.parentCategory)) return false;
...
setupDragAndDrop('studentDropzone', 'studentFileInput', function(csvContent) {
    ...
    for (let i = 1; i < rows.length; i++) {
        ...
        if (cohort.toLowerCase().includes('summer') || cohort.toLowerCase().includes('sip')) cohort = 'Summer';
        else if (cohort.toLowerCase().includes('final')) cohort = 'Final';
        ...
        // Cohort validation
        if (cohort && cohort !== 'Summer' && cohort !== 'Final') {
            isValid = false;
...
setupDragAndDrop('fineDropzone', 'fineFileInput', function(csvContent) {
    ...
    // Standard template fields: Name, Roll Number, Email ID, Cohort, Violation Category, Monetary Penalty Amount, Red Flags, Differentiation Tag, Notes
    const rollIdx = headers.indexOf('roll number');
    const cohortIdx = headers.indexOf('cohort');
    const catIdx = headers.indexOf('violation category');
    const sessionIdx = headers.indexOf('session');
    const amtIdx = headers.indexOf('monetary penalty amount');
    const flagIdx = headers.indexOf('red flags');
    const tagIdx = headers.indexOf('differentiation tag');
    const notesIdx = headers.indexOf('notes');

    if (rollIdx === -1 || catIdx === -1 || sessionIdx === -1) {
        showToast('Invalid CSV format. Roll Number, Violation Category, and Session are mandatory headers.', 'danger');
        return;
    }
...
function executeFineImport() {
    ...
    fineImportBatch.forEach(item => {
        ...
        logFineEdit(fine.id, 'status', null, 'OPEN', 'CREATED', 'Registered via Bulk Upload process');
    });
...
function executePaymentProcessing() {
    ...
    paymentImportBatch.forEach(item => {
        ...
        const updateFineRecord = (fine) => {
            ...
            fine.updated_at = new Date().toISOString();
            closedCount++;
            logFineEdit(fine.id, 'status', oldStatus, item.newStatus, 'STATUS_CHANGED', `${item.closureReason}: ${item.auditComments}`);
        };
